<?php
/*
@file KISA_SEED_CTR.php
@brief SEED CTR
@author Copyright (c) 2013 by KISA
@remarks http://seed.kisa.or.kr/
*/

define('_KISA_CTR_SEED_KEY','00000000000000000000000000000000');	//hashData
define('_KISA_CTR_SEED_COUNTER','00000000000000000000000000000000');//hashData
00EC9584EBB098EC868CED9484ED8AB8
...
class KISA_SEED_INFO {
	...
	function __construct() {
		$this->ctr = array_pad(array(), 4, 0);//ivec -> ctr modi
...
class KISA_SEED_CTR {
	...
	const BLOCK_SIZE_CTR = 16;
	const BLOCK_SIZE_CTR_INT = 4;///add
	...
	static function EncRoundKeyUpdate0(&$T, &$K, $K_offset, &$ABCD, $KC) {
		$T[0] = $ABCD[KISA_SEED_CTR::$ABCD_A];
		$ABCD[KISA_SEED_CTR::$ABCD_A] = (($ABCD[KISA_SEED_CTR::$ABCD_A]>>8)&0x00ffffff) ^ ($ABCD[KISA_SEED_CTR::$ABCD_B]<<24);
		$ABCD[KISA_SEED_CTR::$ABCD_B] = (($ABCD[KISA_SEED_CTR::$ABCD_B]>>8)&0x00ffffff) ^ ($T[0]<<24);
		$T[0] = (($ABCD[KISA_SEED_CTR::$ABCD_A]&0xFFFFFFFF) + ($ABCD[KISA_SEED_CTR::$ABCD_C]&0xFFFFFFFF) - ($KC&0xFFFFFFFF)) & 0xFFFFFFFF;
		$T[1] = (($ABCD[KISA_SEED_CTR::$ABCD_B]&0xFFFFFFFF) + ($KC&0xFFFFFFFF) - ($ABCD[KISA_SEED_CTR::$ABCD_D]&0xFFFFFFFF)) & 0xFFFFFFFF;
		$K[$K_offset+0] = KISA_SEED_CTR::$SS0[KISA_SEED_CTR::GetB0($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS1[KISA_SEED_CTR::GetB1($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS2[KISA_SEED_CTR::GetB2($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS3[KISA_SEED_CTR::GetB3($T[0])&0x0ff];
		$K[$K_offset+1] = KISA_SEED_CTR::$SS0[KISA_SEED_CTR::GetB0($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS1[KISA_SEED_CTR::GetB1($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS2[KISA_SEED_CTR::GetB2($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS3[KISA_SEED_CTR::GetB3($T[1])&0x0ff];
	}

	static function EncRoundKeyUpdate1(&$T, &$K, $K_offset, &$ABCD, $KC) {
		$T[0] = $ABCD[KISA_SEED_CTR::$ABCD_C];
		$ABCD[KISA_SEED_CTR::$ABCD_C] = ($ABCD[KISA_SEED_CTR::$ABCD_C]<<8) ^ (($ABCD[KISA_SEED_CTR::$ABCD_D]>>24)&0x0ff);
		$ABCD[KISA_SEED_CTR::$ABCD_D] = ($ABCD[KISA_SEED_CTR::$ABCD_D]<<8) ^ (($T[0]>>24)&0x0ff);
		$T[0] = (($ABCD[KISA_SEED_CTR::$ABCD_A]&0xFFFFFFFF) + ($ABCD[KISA_SEED_CTR::$ABCD_C]&0xFFFFFFFF) - ($KC&0xFFFFFFFF)) & 0xFFFFFFFF;
		$T[1] = (($ABCD[KISA_SEED_CTR::$ABCD_B]&0xFFFFFFFF) + ($KC&0xFFFFFFFF) - ($ABCD[KISA_SEED_CTR::$ABCD_D]&0xFFFFFFFF)) & 0xFFFFFFFF;
		$K[$K_offset+0] = KISA_SEED_CTR::$SS0[KISA_SEED_CTR::GetB0($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS1[KISA_SEED_CTR::GetB1($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS2[KISA_SEED_CTR::GetB2($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS3[KISA_SEED_CTR::GetB3($T[0])&0x0ff];
		$K[$K_offset+1] = KISA_SEED_CTR::$SS0[KISA_SEED_CTR::GetB0($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS1[KISA_SEED_CTR::GetB1($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS2[KISA_SEED_CTR::GetB2($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS3[KISA_SEED_CTR::GetB3($T[1])&0x0ff];
    ...
    static function SeedRound(&$T, &$LR, $L0, $L1, $R0, $R1, &$K, $K_offset) {
		...
		$T[1] ^= $T[0];
		$T[1] = KISA_SEED_CTR::$SS0[KISA_SEED_CTR::GetB0($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS1[KISA_SEED_CTR::GetB1($T[1])&0x0ff] ^
				KISA_SEED_CTR::$SS2[KISA_SEED_CTR::GetB2($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS3[KISA_SEED_CTR::GetB3($T[1])&0x0ff];
		$T[0] = ($T[0] + $T[1]) & 0x0ffffffff;
		$T[0] = KISA_SEED_CTR::$SS0[KISA_SEED_CTR::GetB0($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS1[KISA_SEED_CTR::GetB1($T[0])&0x0ff] ^
				KISA_SEED_CTR::$SS2[KISA_SEED_CTR::GetB2($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS3[KISA_SEED_CTR::GetB3($T[0])&0x0ff];
	...
	static function BLOCK_XOR_SEED(&$OUT_VALUE, $OUT_VALUE_offset, &$IN_VALUE1, $IN_VALUE1_offset, &$IN_VALUE2, $IN_VALUE2_offset) {
	...
	static function UpdateCounter_for_SEED(&$pbOUT, $pbOUT_offset, $nIncreaseValue, $nMin) {
		$bszBackup = 0;
		$i = 0;
		...
		if( 0 < $nMin ) {
			$b = Common::get_byte_for_int($pbOUT, $pbOUT_offset*4+$nMin);
			$bszBackup = $b & 0x0ff;
			Common::set_byte_for_int($pbOUT, $pbOUT_offset*4+$nMin, ($b + $nIncreaseValue));
		}

		for( $i=$nMin; $i>1; --$i )
		{
			if( $bszBackup <= (((int)Common::get_byte_for_int($pbOUT, $pbOUT_offset*4+$i)) & 0x0ff) ) {
				return;
			} else {
				$b = Common::get_byte_for_int($pbOUT, $pbOUT_offset*4+$i-1);
				$bszBackup = $b & 0x0ff;
				Common::set_byte_for_int($pbOUT, $pbOUT_offset*4+$i-1, ($b + 1));
			}
		}

		$b = Common::get_byte_for_int($pbOUT, $pbOUT_offset*4+0);
		$bszBackup = $b & 0x0ff;
		Common::set_byte_for_int($pbOUT, $pbOUT_offset*4+0, ($b + $nIncreaseValue));
	}

	static function chartoint32_for_SEED_CTR(&$in, $inLen) {
		$data = null;
		$len=0; $i=0;
	...
	static function int32tochar_for_SEED_CTR(&$in, $inLen) {
		$data = null;
		$i = 0;
	...
	static function KISA_SEED_Encrypt_Block_forCTR(&$in, $in_offset, &$out, $out_offset, &$ks) {
		...
		if(_KISA_COMMON_IS_LITTLE_ENDIAN == false) {
			$LR[LR_L0] = KISA_SEED_CTR::EndianChange($LR[LR_L0]);
			$LR[LR_L1] = KISA_SEED_CTR::EndianChange($LR[LR_L1]);
			$LR[LR_R0] = KISA_SEED_CTR::EndianChange($LR[LR_R0]);
			$LR[LR_R1] = KISA_SEED_CTR::EndianChange($LR[LR_R1]);
		}

		KISA_SEED_CTR::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K,  0);    // Round 1
		KISA_SEED_CTR::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K,  2);    // Round 2
		KISA_SEED_CTR::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K,  4);    // Round 3
		KISA_SEED_CTR::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K,  6);    // Round 4
		KISA_SEED_CTR::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K,  8);    // Round 5
		KISA_SEED_CTR::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 10);    // Round 6
		KISA_SEED_CTR::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 12);    // Round 7
		KISA_SEED_CTR::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 14);    // Round 8
		KISA_SEED_CTR::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 16);    // Round 9
		KISA_SEED_CTR::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 18);    // Round 10
		KISA_SEED_CTR::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 20);    // Round 11
		KISA_SEED_CTR::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 22);    // Round 12
		KISA_SEED_CTR::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 24);    // Round 13
		KISA_SEED_CTR::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 26);    // Round 14
		KISA_SEED_CTR::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 28);    // Round 15
		KISA_SEED_CTR::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 30);    // Round 16
	...
	static function SEED_CTR_init(&$pInfo, $enc, &$pbszUserKey, &$pbszCTR )
	{
		...
		$pInfo->encrypt = $enc;
		Common::memcpy_byte2int($pInfo->ctr, $pbszCTR, 16);//ivec -> ctr modi
		$pInfo->last_block_flag = $pInfo->buffer_length = 0;

		$ABCD[KISA_SEED_CTR::$ABCD_A] = Common::get_byte_to_int($pbszUserKey, 0*4);
		$ABCD[KISA_SEED_CTR::$ABCD_B] = Common::get_byte_to_int($pbszUserKey, 1*4);
		$ABCD[KISA_SEED_CTR::$ABCD_C] = Common::get_byte_to_int($pbszUserKey, 2*4);
		$ABCD[KISA_SEED_CTR::$ABCD_D] = Common::get_byte_to_int($pbszUserKey, 3*4);

		if(_KISA_COMMON_IS_LITTLE_ENDIAN == false) {
			$ABCD[KISA_SEED_CTR::$ABCD_A] = KISA_SEED_CTR::EndianChange($ABCD[KISA_SEED_CTR::$ABCD_A]);
			$ABCD[KISA_SEED_CTR::$ABCD_B] = KISA_SEED_CTR::EndianChange($ABCD[KISA_SEED_CTR::$ABCD_B]);
			$ABCD[KISA_SEED_CTR::$ABCD_C] = KISA_SEED_CTR::EndianChange($ABCD[KISA_SEED_CTR::$ABCD_C]);
			$ABCD[KISA_SEED_CTR::$ABCD_D] = KISA_SEED_CTR::EndianChange($ABCD[KISA_SEED_CTR::$ABCD_D]);
		}

		$T[0] = (($ABCD[KISA_SEED_CTR::$ABCD_A]&0xFFFFFFFF) + ($ABCD[KISA_SEED_CTR::$ABCD_C]&0xFFFFFFFF) - KISA_SEED_CTR::$KC0) & 0x0ffffffff;
		$T[1] = (($ABCD[KISA_SEED_CTR::$ABCD_B]&0xFFFFFFFF) - ($ABCD[KISA_SEED_CTR::$ABCD_D]&0xFFFFFFFF) + KISA_SEED_CTR::$KC0) & 0x0ffffffff;
		$K[0] = (KISA_SEED_CTR::$SS0[KISA_SEED_CTR::GetB0($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS1[KISA_SEED_CTR::GetB1($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS2[KISA_SEED_CTR::GetB2($T[0])&0x0ff] ^ KISA_SEED_CTR::$SS3[KISA_SEED_CTR::GetB3($T[0])&0x0ff]) & 0x0ffffffff;
		$K[1] = (KISA_SEED_CTR::$SS0[KISA_SEED_CTR::GetB0($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS1[KISA_SEED_CTR::GetB1($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS2[KISA_SEED_CTR::GetB2($T[1])&0x0ff] ^ KISA_SEED_CTR::$SS3[KISA_SEED_CTR::GetB3($T[1])&0x0ff]) & 0x0ffffffff;

		KISA_SEED_CTR::EncRoundKeyUpdate0($T, $K,  2, $ABCD, KISA_SEED_CTR::$KC1 );
		KISA_SEED_CTR::EncRoundKeyUpdate1($T, $K,  4, $ABCD, KISA_SEED_CTR::$KC2 );
		KISA_SEED_CTR::EncRoundKeyUpdate0($T, $K,  6, $ABCD, KISA_SEED_CTR::$KC3 );
		KISA_SEED_CTR::EncRoundKeyUpdate1($T, $K,  8, $ABCD, KISA_SEED_CTR::$KC4 );
		KISA_SEED_CTR::EncRoundKeyUpdate0($T, $K, 10, $ABCD, KISA_SEED_CTR::$KC5 );
		KISA_SEED_CTR::EncRoundKeyUpdate1($T, $K, 12, $ABCD, KISA_SEED_CTR::$KC6 );
		KISA_SEED_CTR::EncRoundKeyUpdate0($T, $K, 14, $ABCD, KISA_SEED_CTR::$KC7 );
		KISA_SEED_CTR::EncRoundKeyUpdate1($T, $K, 16, $ABCD, KISA_SEED_CTR::$KC8 );
		KISA_SEED_CTR::EncRoundKeyUpdate0($T, $K, 18, $ABCD, KISA_SEED_CTR::$KC9 );
		KISA_SEED_CTR::EncRoundKeyUpdate1($T, $K, 20, $ABCD, KISA_SEED_CTR::$KC10);
		KISA_SEED_CTR::EncRoundKeyUpdate0($T, $K, 22, $ABCD, KISA_SEED_CTR::$KC11);
		KISA_SEED_CTR::EncRoundKeyUpdate1($T, $K, 24, $ABCD, KISA_SEED_CTR::$KC12);
		KISA_SEED_CTR::EncRoundKeyUpdate0($T, $K, 26, $ABCD, KISA_SEED_CTR::$KC13);
		KISA_SEED_CTR::EncRoundKeyUpdate1($T, $K, 28, $ABCD, KISA_SEED_CTR::$KC14);
		KISA_SEED_CTR::EncRoundKeyUpdate0($T, $K, 30, $ABCD, KISA_SEED_CTR::$KC15);
	...
	//	$pdwCounter_offset = 0;
		...
		$pdwCounter = &$pInfo->ctr; //ivec -> ctr modi
		//$in_offset = 0;			del
		//$out_offset = 0;			del
		//$pbszCounter_offset = 0;  del
		//$pdwCounter_offset = 0;   del

		while( $nCurrentCount < $inLen )
		{
			KISA_SEED_CTR::KISA_SEED_Encrypt_Block_forCTR( $pdwCounter, 0, $out, $out_offset, $pInfo->seed_key );
			KISA_SEED_CTR::BLOCK_XOR_SEED( $out, $out_offset, $in, $in_offset, $out, $out_offset );

			$pbszCounter = $pdwCounter;
		//	$pbszCounter_offset = $pdwCounter_offset; //del
			KISA_SEED_CTR::UpdateCounter_for_SEED( $pdwCounter, 0, 1, (KISA_SEED_CTR::BLOCK_SIZE_CTR-1) );

			$nCurrentCount += KISA_SEED_CTR::BLOCK_SIZE_CTR;
			$in_offset += KISA_SEED_CTR::BLOCK_SIZE_CTR_INT;//modi
			$out_offset += KISA_SEED_CTR::BLOCK_SIZE_CTR_INT;//modi
		}
	...
	static function SEED_CTR_Close( &$pInfo, &$out, $out_offset, &$outLen ) {
		$nPaddngLeng = - $pInfo->buffer_length;
	...
	static function SEED_CTR_Encrypt( &$pbszUserKey, &$pbszCTR, &$message, $message_offset, $message_length ) {
		...
		$nPlainTextPadding = (KISA_SEED_CTR::BLOCK_SIZE_CTR - ($nPlainTextLen % KISA_SEED_CTR::BLOCK_SIZE_CTR)) % KISA_SEED_CTR::BLOCK_SIZE_CTR;
		...
		$pbszCipherText = array_pad(array(), $nPlainTextLen, 0);

		KISA_SEED_CTR::SEED_CTR_init( $info, KISA_ENC_DEC::KISA_ENCRYPT, $pbszUserKey, $pbszCTR );

		$outlen = ((int)(count($newpbszPlainText)/16)) *4;
		$outbuf = array_pad(array(), $outlen, 0);
		$data = KISA_SEED_CTR::chartoint32_for_SEED_CTR($pbszPlainText, $nPlainTextLen);

		KISA_SEED_CTR::SEED_CTR_Process( $info, $data, $nPlainTextLen, $outbuf, $nOutLeng );
		KISA_SEED_CTR::SEED_CTR_Close($info,$outbuf,$nOutLeng,$nEncRmainLeng);//추가
		$cdata = KISA_SEED_CTR::int32tochar_for_SEED_CTR($outbuf, $nOutLeng-$nEncRmainLeng);//수정
		Common::arraycopy($pbszCipherText, $cdata, $nOutLeng-$nEncRmainLeng);//수정
...
function seed_encrypt($str) {

	$str = unpack('H*',$str);
	preg_match_all('/[A-Fa-f0-9]./',array_shift($str),$data_array1);
	preg_match_all('/[A-Fa-f0-9]./',_KISA_CTR_SEED_KEY,$data_array2);
	preg_match_all('/[A-Fa-f0-9]./',_KISA_CTR_SEED_COUNTER,$data_array3);

	$planBytes = $data_array1[0];
	$keyBytes = $data_array2[0];
	$CTRBytes = $data_array3[0];
...
function seed_decrypt($str) {
	$str = base64_decode($str);

	preg_match_all('/[A-Fa-f0-9]./',$str,$data_array1);
	preg_match_all('/[A-Fa-f0-9]./',_KISA_CTR_SEED_KEY,$data_array2);
	preg_match_all('/[A-Fa-f0-9]./',_KISA_CTR_SEED_COUNTER,$data_array3);
